...
<body>
					...
					<table class="table">
						...
						<tbody>
							<c:forEach var="item" items="${cartItems}">
								<tr>
									<td><input type="checkbox" name="cartItemIds"
										value="${item.product_id}"> <input type="hidden"
										name="price_${item.product_id}" value="${item.product_price}">
										<input type="hidden" name="quantity_${item.product_id}"
									...
									<td><input type="number"
										...
										value="${item.quantity}"
										style="border: 1px solid gray; width: 50px;"
										onchange="updateItemQuantity('${item.product_id}', this.value)">
									</td>
									<td id="subtotal_${item.product_id}">${item.product_price * item.quantity}
										원</td>
									<td><a href="#" class="btn-delete"
										onclick="deleteCartItem('${item.product_id}')">삭제</a></td>
								</tr>
							</c:forEach>
					...
					<div class="space-y-1">
						<div class="flex justify-between">
							<span class="text-sm">총 상품 가격</span> <span
								class="text-sm font-medium" id="totalPrice">${totalPrice}
								원</span>
						</div>
						<div class="flex justify-between">
							<span class="text-sm">배송비</span> <span
								class="text-sm font-medium">0 원</span>
						</div>
						<div class="flex justify-between">
							<span class="text-sm">총 주문 상품수</span> <span
								class="text-sm font-medium" id="totalQuantity">${totalQuantity}
								건</span>
						</div>
					</div>
					<div class="space-y-1">
						<div class="flex justify-between">
							<span class="text-sm">총 결제 예정 금액</span> <span
								class="text-sm font-medium" id="totalPayment">${totalPrice}
								원</span>
						</div>
					</div>
				</div>
	...
	<script>
        ...
        function updateItemQuantity(productId, quantity) {
            $.ajax({
                url: '/sms/updateItemQuantity',
                type: 'POST',
                data: { product_id: productId, quantity: quantity },
                success: function(response) {
                    // Update the subtotal for the changed item
                    const price = parseInt($(`input[name="price_${productId}"]`).val(), 10);
                    const subtotal = price * quantity;
                    $(`td#subtotal_${productId}`).text(subtotal + ' 원');
        ...
        function updateCartSummary() {
            ...
            $('#totalPayment').text(totalPrice.toLocaleString() + ' 원');
        }
